...
class EventOps
  def findEventsNear(lat, lon, maxDist=5000)
    ...
    @eventList.each { |event|
      ...
      if MathOps.distance(lat, lon, elat, elon) < maxDist
        events << event
      end
    }
  ...
  def findEarthquakesNear(cityName)
    ...
    @eventList.each { |event|
      ...
      if type == "earthquake"
        if MathOps.distance(elat, elon, clat, clon) < 5000
          events << event
        end
      end
    }
...
# What "kinds" of data are there in this code? Can you identify similar and different data?
# Likewise, what "kinds" of functionality is in here?
# How many different levels of abstraction are there? This is a hard question. Hint: maybe the individual events are one level of abstraction, and operations that work on collections of events are at another...?

# Looks for natural disasters near a given city.
# Adds new events to the event list.
# This class has a lot of responsibilities. There is map data, event data, calculation data, etc.
# There are lots of functionality here: distance calculations, searching/filtering, adding new events.
# I think there are 3 levels of abstraction. 